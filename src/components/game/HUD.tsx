...
import { Coins, Zap, MapPin, Pause, Play } from 'lucide-react';
    ...
    <div className="absolute top-0 left-0 w-full p-4 flex flex-col gap-4 pointer-events-none z-40">
      <div className="flex justify-between items-start">
        ...
        <div className="flex flex-col items-center">
          <div className="flex items-center gap-2 px-6 py-2 bg-primary/20 backdrop-blur-xl rounded-full border-2 border-primary/40 shadow-[0_0_20px_rgba(37,99,235,0.3)] pointer-events-auto">
            <MapPin className="w-5 h-5 text-primary fill-primary animate-pulse" />
            <div className="text-3xl font-black text-white text-glow italic tracking-tighter tabular-nums">
              {state.distance.toLocaleString()}m
            </div>
          </div>
          <div className="mt-2 text-[10px] font-bold uppercase tracking-[0.3em] text-white/50 drop-shadow-md">
            Campaign Distance
          </div>
        </div>
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="flex flex-col items-end bg-accent/20 backdrop-blur-md border border-accent/40 p-3 rounded-2xl shadow-lg">
            <span className="text-[10px] font-bold uppercase tracking-widest text-white/50">Phase</span>
          ...
          <div className="flex items-center gap-1.5 text-xs font-bold text-white/60 bg-black/40 px-3 py-1 rounded-full">
            <Zap className="w-4 h-4 text-primary fill-primary" /> {state.score.toLocaleString()}
          </div>
        </div>
      </div>
      ...
      <div className="w-full max-w-sm mx-auto mt-4 transform translate-y-[-10px]">
        <div className="flex justify-between items-end mb-1.5 px-2">
          <span className="text-[10px] font-bold text-white/70 uppercase tracking-widest">Challenges {state.questionsInLevel}/{QUESTIONS_PER_LEVEL}</span>
